    ...
    render(){
        ...
        <form onSubmit={this.props.handleSubmit}>
            <select className="nodeInput" type="text" name={"firstNode" + this.props.type + "Value"} placeholder="first node" value={this.props.Node1InputValue} onChange={this.props.handleNode1InputChange} disabled={!this.props.nodesCreated || this.props.simulating}>
                { this.props.nodesNumber && nodes.map((item, index) => <option value={index.toString()} key={index}> {item} </option>) }
            </select>
            ...
            <input className="textInput" type="text"  placeholder={this.props.placeholder} value={this.props.InputValue} onChange={this.props.handleInputChange} disabled={!this.props.nodesCreated || this.props.simulating}/>
            <input className="button" type="submit" value={"create " + this.props.type} disabled={!this.props.nodesCreated || this.props.simulating}/>
        </form> 
        )
    }
}
